...
gripper_forward_direction_camera = torch.tensor([[1., 0., 1.]]).to(device)
...
for model_epoch in range(eval_conf.start_epoch, eval_conf.end_epoch + 1):
    ...
    with torch.no_grad():
        ...
        if eval_conf.AIP:
            AIP_score = AIP.inference_score(pc, dirs1, dirs2, torch.sigmoid(net)).cpu().numpy()
            print('AIP_score: ', AIP_score.shape)
            AIP_score *= pc_movable
            fn = os.path.join(result_dir, 'AIP')
            utils.render_pts_label_png(fn, pc[0].cpu().numpy(), AIP_score)
        ...
        fn = os.path.join(result_dir, 'confidence')
        utils.render_pts_label_png(fn, pc[0].cpu().numpy(), confidence)


if eval_conf.render_gt:
    ...
    for pt_id, pt in enumerate(pcids):
        # set to original state
        if pt not in pcids[pc_movable]:
            continue
    ...
    fn = os.path.join(result_dir, 'gt')
    gt_label *= pc_movable
    utils.render_pts_label_png(fn,  pc[0].cpu().numpy(), gt_label)

    fn = os.path.join(result_dir, 'loss')
    loss = network.critic.get_ce_loss(pred, torch.tensor(gt_label)).numpy()
    loss *= pc_movable
    utils.render_pts_label_png(fn,  pc[0].cpu().numpy(), loss)
